...
export default function Navigation({ isAuthenticated, onGalleryClick, onCommissionClick, onHomeClick, onProfileClick, onLoginClick, onSignupClick, onLogoutClick }: NavigationProps) {
  return (
    <header style={{ position: "sticky", top: 0, zIndex: 10, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px", background: "rgba(255, 247, 245, 0.95)", backdropFilter: "blur(8px)", borderBottom: "1px solid #ead9d2" }}>
      ...
      <nav style={{ display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "flex-end" }}>
        ...
        {isAuthenticated ? <button onClick={onProfileClick} style={{ border: "none", background: "transparent", padding: 0, color: "#6a4b43", fontWeight: 600, cursor: "pointer" }}>Profile</button> : <button onClick={onLoginClick} style={{ border: "none", background: "transparent", padding: 0, color: "#6a4b43", fontWeight: 600, cursor: "pointer" }}>Sign In</button>}
        {isAuthenticated ? <button onClick={onLogoutClick} style={{ border: "none", background: "transparent", padding: 0, color: "#6a4b43", fontWeight: 600, cursor: "pointer" }}>Logout</button> : <button onClick={onSignupClick} style={{ border: "none", background: "transparent", padding: 0, color: "#6a4b43", fontWeight: 600, cursor: "pointer" }}>Sign Up</button>}
      </nav>
    </header>
  );
}
