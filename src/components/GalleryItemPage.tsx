            ...
            <div style={{ overflow: "hidden", border: "1px solid var(--line)", borderRadius: 8, background: "var(--bg-gallery-item)", boxShadow: "0 10px 30px rgba(31, 51, 40, 0.08)" }}>
              {activeImage ? (
                <div style={{ overflow: "hidden" }}>
                  <img src={activeImage.imageUrl} alt={item.title} style={{ display: "block", width: "100%", aspectRatio: "4 / 3", objectFit: "cover", background: "var(--linen)", transition: "transform 220ms ease" }} onMouseEnter={(event) => { event.currentTarget.style.transform = "scale(1.16)"; }} onMouseLeave={(event) => { event.currentTarget.style.transform = "scale(1)"; }} />
                </div>
              ) : null}
            </div>
